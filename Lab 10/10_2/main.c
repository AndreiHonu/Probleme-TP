...
void Propozitie_inserare(Propozitie *p,Cuvant *c,const char *text)
{
    Cuvant *b;
    for(b=c;b;b=b->urm){
        b->urm = b;
        }
    strcpy(c->urm->text,c->text);
    strcpy(c->text,text);
    ...
    do{
        ...
        switch(op){
            ...
            case 4:
                ...
                scanf("%s",text);
                printf("succesorul cuvantului: ");
                scanf("%s",succ);
                c = Propozitie_cauta(&p,succ);
                if(c)
                {
                    Propozitie_inserare(&p,c,text);
                }
                else
                {
                    Cuvant *c= Cuvant_nou(text);
